...
namespace BzKovSoft.ObjectSlicer.EventHandlers
{
	...
	class BzFixMassSmart : MonoBehaviour, IBzObjectSlicedEvent
	{
		...
		IEnumerator NextFrame(GameObject original, GameObject resultNeg, GameObject resultPos)
		{
			...
			rigidA.mass = massO * VolumeOfMesh(resultNeg, out centerOfMassA);
			rigidB.mass = massO * VolumeOfMesh(resultPos, out centerOfMassB);
			rigidA.mass = massO * VolumeOfMesh(resultNeg, out centerOfMassA);
			rigidB.mass = massO * VolumeOfMesh(resultPos, out centerOfMassB);
			rigidA.centerOfMass = Vector3.Scale(centerOfMassA, resultNeg.transform.lossyScale);
            rigidB.centerOfMass = Vector3.Scale(centerOfMassB, resultNeg.transform.lossyScale);
		}
